...
def test_board_encoding_performance() -> None:
    ...
    assert time_per_encoding < 0.001, f"Board encoding too slow: {time_per_encoding*1000:.3f}ms per encoding"
...
def test_legal_moves_performance() -> None:
    ...
    assert time_per_call < 0.01, f"get_legal_moves too slow: {time_per_call*1000:.3f}ms per call"
...
def test_place_move_performance() -> None:
    ...
    assert time_per_operation < 0.001, f"place_move+undo too slow: {time_per_operation*1000:.3f}ms per operation"
...
def test_calculate_score_performance() -> None:
    ...
    assert time_per_calculation < 0.002, f"calculate_score too slow: {time_per_calculation*1000:.3f}ms per calculation"
